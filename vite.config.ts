...
function metaProxyPlugin(): Plugin {
  return {
    name: 'videovault-meta-proxy',
    configureServer(server) {
      server.middlewares.use('/api/meta', async (req: any, res: any) => {
        const url = new (globalThis as any).URL(req.url ?? '', 'http://localhost').searchParams.get('url');
        if (!url) {
          res.statusCode = 400;
